...
const LCSwiftDocsUploader = () => {
  ...
  const startWebSocketMonitoring = (id) => {
    ...
    try {
      ...
      const wsUrl = `wss://192.168.18.62:50013/lc/ws/progress/${id}?token=${encodeURIComponent(token)}`;
  ...
  const checkProgressManually = async (id) => {
    try {
      ...
      const response = await fetch(`https://192.168.18.62:50013/lc/progress/${id}`, {
        method: 'GET',
        headers: {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://192.168.18.62:50013/lc/upload_docs/`, {
        method: 'POST',
        headers: {
          'accept': 'application/json',
          'Authorization': `Bearer ${token}`
