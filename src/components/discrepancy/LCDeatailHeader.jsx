...
const Header = ({ 
  ...
  navigate, 
  onBackToDashboard, 
  onAction, // Changed from onGenerateReport to onAction
  userRole 
}) => {
  // Determine button text based on role
  const getButtonText = () => {
  ...
  // Determine tooltip text based on role
  const getTooltipText = () => {
    if (!allTabsVisited) {
      return "Please review all documents first";
    }
      ...
      <div className="w-full px-6 py-4">
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center space-x-3">
            ...
            <button
              onClick={onAction} // Changed from onGenerateReport to onAction
              disabled={!allTabsVisited}
              className={`inline-flex items-center px-12 py-3 border border-white text-sm font-medium rounded-md shadow-md text-white ${
                allTabsVisited
                  ? 'bg-indigo-900 hover:bg-[#646cffaa]/90'
