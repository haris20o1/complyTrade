...
const Sidebar = ({
  ...
  return (
    <div className="w-64 fixed h-screen bg-gray-300 border-r border-gray-200 min-h-screen">
      ...
      <div className="p-4 border-b border-gray-200">
        <button
          className={`w-full text-left px-3 py-2 rounded-md text-sm font-medium transition-colors flex items-center justify-between ${currentView === 'lc-overview'
              ? 'bg-indigo-900 text-white'
              : 'text-gray-700 hover:bg-indigo-100 hover:text-indigo-900'
            }`}
          onClick={() => onViewChange('lc-overview')}
        >
          <div className="flex items-center">
            <FileText className="h-4 w-4 mr-2" />
            <span>Scrutiny Overview</span>
          </div>
          <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${currentView === 'lc-overview' ? 'bg-white/20 text-white' : 'bg-gray-200 text-gray-700'
            }`}>
            {mainPoints.reduce((total, point) => total + point.subPoints.filter(sp => sp.status === 'discripant').length, 0)}
          </span>
        </button>
      </div>
