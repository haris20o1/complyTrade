  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        // Filter to only show compliance managers
        const complianceManagers = data.filter(user => user.role === 'complyce_manager');
        
        setUsers(complianceManagers);
        console.log(`Successfully fetched ${complianceManagers.length} compliance managers out of ${data.length} total users`);
      } catch (err) {
      ...
      <div className="py-6">
        <div className="mx-auto px-4 sm:px-6 md:px-8">
          ...
          <p className="mt-2 text-sm text-gray-600">
            Monitor performance metrics and activity for compliance managers in the system.
          </p>
        </div>
        ...
        {loading ? (
          <div className="mx-auto px-4 sm:px-6 md:px-8 py-16 flex justify-center">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-gray-500">Loading compliance managers...</span>
            </div>
          </div>
        ) : error && !selectedUser ? (
              ...
              <div className="lg:w-1/3">
                <div className="bg-white shadow rounded-lg overflow-hidden">
                  <div className="px-4 py-5 border-b border-gray-200">
                    <h3 className="text-lg leading-6 font-medium text-gray-900">Compliance Managers</h3>
                    <p className="mt-1 text-sm text-gray-500">
                      Select a compliance manager to view their performance metrics
                    </p>
                  </div>
                ...
                {!selectedUser ? (
                  <div className="bg-white shadow rounded-lg p-6 flex items-center justify-center h-64">
                    <div className="text-center">
                      ...
                      <h3 className="mt-2 text-lg font-medium text-gray-900">No Compliance Manager Selected</h3>
                      <p className="mt-1 text-sm text-gray-500">
                        Select a compliance manager from the list to view their performance details
                      </p>
                    </div>
                  </div>
                ) : loadingPerformance ? (
